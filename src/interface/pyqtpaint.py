...
class PyQtPaint(QtGui.QWidget):
    ...
    def __init__(self, width, height, *args, **kwargs):
        ...
        uic.loadUi('interface/ui/pyqtpaint.ui', self)
    ...
    def _create_actions(self):
        ...
        self.predict_action = QtGui.QAction('Predict', self)
    ...
    def _make_connections(self):
        ...
        self.predict_action.triggered.connect(self.predict_img)
    ...
    def predict_img(self):
        ...
        model.load_weights("data/mlp.h5")
        img = self.get_img()
        img.save('predict.png')
        with open('predict.png', 'r+b') as f:
            with Image.open(f) as image:
                    cover = resizeimage.resize_cover(image, [20, 20]).convert('L')
                    cover.save('predict.png', image.format)
        img = mpimg.imread('predict.png').flatten()
        img = np.vstack((img, img))
        y = model.predict(img)
        output = y.argmax(axis=1)
        print('Predicted Digit: {} ({})'.format(output[0], y[0][output[0]]))
